...
public class InquiryFragment extends Fragment implements Serializable {
...
//    TextView mResultText;
    ...
    @OnClick({R.id.inquiry_add_button})
    void click(View v) {
        switch (v.getId()) {
            case R.id.inquiry_add_button:
                CommonLib.subActivityIntent(getActivity(),InquiryAddFragment.newInstance());
                break;
        }
    }
...
//                EventBus.getDefault().post(new FragmentReplaceEvent(MessageDetailsFragment.newInstance(),"상세보기"));
                CommonLib.subActivityIntent(getActivity(),InquiryDetailsFragment.newInstance());
